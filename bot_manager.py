    ...
    def _process_msg(self, msg:mitm.WSMessage):
        ...
        if msg.type == mitm.WsType.START:
            ...
            if liqi_method in METHODS_TO_IGNORE:
                ...
                (liqi.MsgType.RES, liqi.LiqiMethod.oauth2Login),
                (liqi.MsgType.RES, liqi.LiqiMethod.login),
                (liqi.MsgType.RES, liqi.LiqiMethod.fetchAccountInfo),
                ):
                # lobby login msg
                if self.lobby_flow_id is None:  # record first time in lobby
                    ...
                    if(self.lobby_flow_id != msg.flow_id):
