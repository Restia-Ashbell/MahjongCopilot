...
class Automation:
    ...
    def automate_action(self, mjai_action:dict, game_state:GameState) -> bool:
        ...
        # "button" action
            ...
            more_steps:list[ActionStep] = self.steps_button_action(mjai_action, gi, game_state)
    ...
    def steps_button_action(self, mjai_action:dict, gi:GameInfo, game_state:GameState) -> list[ActionStep]:
        """Generate action steps for button actions (chi, pon, kan, etc.)"""       
        if 'operationList' not in game_state.last_operation:   # no liqi operations provided - no buttons to click
            return []
        
        # add None/Pass button
